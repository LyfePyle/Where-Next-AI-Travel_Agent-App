...
/** Match a stop preview by destination name or index. */
export function getStopPreviewForDestination(
  ...
  const cityPart = norm.split(',')[0]?.trim();
  if (cityPart) {
    const byCity = stopPreviews.find((sp) =>
      sp.destination.toLowerCase().trim().startsWith(cityPart)
    );
    if (byCity) return byCity;
  }

  return stopPreviews[index];
}
